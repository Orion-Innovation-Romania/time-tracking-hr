...
>(({ className, children, position = 'popper', ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        'relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-md border bg-card text-card-foreground shadow-md data-[state=open]:animate-fade-in',
...
>(({ className, children, description, ...props }, ref) => (
  <SelectPrimitive.Item
    ...
    className={cn(
      ...
      description && 'h-auto items-start whitespace-normal py-2',
      className,
    )}
    {...props}
  >
    <span className={cn('absolute left-2 flex h-3.5 w-3.5 items-center justify-center', description && 'top-2.5')}>
